...
export const listPolls = createServerFn({ method: "GET" }).handler(
  async (): Promise<PublicPoll[]> => {
    ...
    const { data } = await publicClient()
      .from("polls")
      .select("id, code, title, author_display_name, description, explanation, yes_count, no_count, created_at")
...
export const createPoll = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<{ code: string }> => {
    ...
    // Abuse protection: max 5 new polls per account per day.
    ...
    if (!code) throw new Error("Could not allocate a poll link. Try again.");
...
export const castVote = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<{ yes: number; no: number }> => {
    ...
    if (!poll.milestone_notified && poll.yes_count > 1000) {
      // Claim the one-time flag first so the email can never be sent twice.
      ...
      if (claimed) {
        const { data: privateRow } = await supabaseAdmin
          ...
          .eq("poll_id", poll.id)
          .maybeSingle();
        if (privateRow?.author_email) {
          const { sendMilestoneEmail } = await import("./emails.server");
          const { siteOrigin } = await import("./server-shared.server");
          ...
          if (!sent) {
            // Allow a retry on a later vote if the mail could not be delivered.
            await supabaseAdmin
              .from("polls")
              .update({ milestone_notified: false })
              .eq("id", poll.id);
          }
        }
      }
...
function mapPoll(row: PollRow): PublicPoll {
  return {
    id: row.id,
    code: row.code,
    title: row.title,
